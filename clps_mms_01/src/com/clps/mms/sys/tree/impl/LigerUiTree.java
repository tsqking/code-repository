 ...
 * @author Charles.Tan
 */
public class LigerUiTree implements ITreeVo {
	...
	private List<ITreeVo> children = new ArrayList<ITreeVo>();
	...
	public LigerUiTree(String id, String name, String url) {
		if (id == null || name == null || url == null)
			throw new RuntimeException("id name url都不能为空");
	...
	public LigerUiTree(String id, String name, String url, ITreeVo parent) {
		this(id, name, url);
		this.parent = parent;
		hasChange = true;
	}

	public LigerUiTree(String id, String name, String url, List<ITreeVo> children) {
		this(id, name, url);
	...
	/**
	 * 这里同时会计算树的层次 并且这里会同时维护parant -
	 * children之间的关联管理，也就是在设置当前节点的子节点时，同时会指点这些子节点的父亲节点为当前节点
	 */
	@Override
	public void setChildren(List<ITreeVo> children) {
		if (children == null || children.size() == 0)
			return; // 如果为null，do nothing
	 ...
	 * @see MenuVo#toJson()
	 */
	private String toJson(ITreeVo tree) {
		JSONObject json = new JSONObject();
		if (tree.children() != null && tree.children().size() != 0) {
			...
			if (tree.children() != null) {
				for (ITreeVo child : tree.children()) {
					// 让每个子menu递归的去生成json数据
					childrenJson.add(toJson(child));
				}
				json.put("children", childrenJson);
	...
	public List<ITreeVo> route() {
		List<ITreeVo> route = new ArrayList<ITreeVo>();
		ITreeVo current = this;
		while (current != null) {
			route.add(current);
	...
	public ITreeVo subTree(int position) {
		if (position < 0)
			throw new RuntimeException("position 小于0");
	...
	public boolean equals(Object obj) {
		return id.equals(((ITreeVo) obj).id());
	}
	...
	public String toString() {
		return "LigerUiTree [" + "id=" + id + ", name=" + name + ", level=" + level + ", url=" + url + ", nodeType="
				+ nodeType() + ", isexpand=" + isexpand + ",  pic=" + pic + "]";
	}
	 ...
	 */
	private void calculateLevel0(ITreeVo subTree) {
	...
	public boolean deleteSubTree(int position) {
		if (position < 0 || children == null || children.size() == 0 || position >= children.size())
			return false;
		ITreeVo tree = children.remove(position);
		if (children.size() > 0)
			nodeType = MENU;
	...
	private void calculateLevel() {
		...
		for (int i = 0; i < children.size(); i++) {
			...
			if (children.get(i).level() > max)
				max = children.get(i).level();
		}
	...
	public boolean isMenu() {
		return nodeType.equals(ITreeVo.MENU);
	}

	@Override
	public boolean isLeaf() {
		return nodeType.equals(ITreeVo.LEAF);
	}
