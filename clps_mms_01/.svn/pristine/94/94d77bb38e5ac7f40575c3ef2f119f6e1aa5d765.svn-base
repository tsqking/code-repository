$(document).ready(function(){
	...
	var validate = $("#signupForm").validate({
		...
		messages: {
			email: {
				required: $.i18n.prop("PleaseInputEmail"),
				email: $.i18n.prop("PleaseInputCorrectEmail")
			},
			password: {
				required: $.i18n.prop("PleaseInputPassword"),
				minlength: jQuery.format($.i18n.prop("PasswordFormat")),
				maxlength: jQuery.format($.i18n.prop("PasswordFormatMax"))
			},
			passwordAgain: {
				required: $.i18n.prop("PasswordAgain"),
				minlength: jQuery.format($.i18n.prop("PasswordFormat")),
				maxlength: jQuery.format($.i18n.prop("PasswordFormatMax")),
				equalTo: jQuery.format($.i18n.prop("PasswordDifferent"))
			},
			nickname: {
				required: $.i18n.prop("PleaseInputnickname")
			},
			username: {
				required: $.i18n.prop("PleaseInputusername")
			},
			tel: {
				...
				success: function(data){
					...
					if(data.hasOwnProperty("code")){
						if(data.code == 0){
							...
							$("#username").next("label.repeated").text($.i18n.prop("Error.CompaniesAlreadyExists"));
							registError.show();
						}else if(data.code == 4){
							//邮箱已经被注册
