$(function() {
	...
	function blackQueryParams(params) { // 配置参数
	...
	$('#myBlackTbl').bootstrapTable({
		method : "post",// 请求方式（*）
		url : "myblackList.do",// 请求后台的URL（*）
		queryParams : blackQueryParams,// 传递参数（*)
		uniqueId : "b_friend_nickname",// 唯一标识
		pageSize : 10,// 每页的记录行数（*）
		pageList : [ 5, 10, 15, 20, 25, 30 ],// 可供选择的每页的行数（*）
		pageNumber : 1,// 初始化加载第一页，默认第一页
		pagination : true,// 是否显示分页（*）
		sidePagination : 'server',// 分页方式：client客户端分页，server服务端分页（*）
		sortName : "b_date",// 排序字段
		sortOrder : "desc",// 排序方式
		showColumns : true, // 是否显示所有的列
		toolbar : "#toolbar_myblack",// 工具按钮用哪个容器
	...
	// 触发按钮点击事件，多条件模糊搜索
	$("#blackSearchBtn").on("click", function() {
		$('#myBlackTbl').bootstrapTable("refresh", {
			silent : true
		});
	});
	...
	// 从黑名单移除
	$("#btn_removeBlack").click(function() {
		var rows = $('#myBlackTbl').bootstrapTable('getAllSelections');
		var json = [];
