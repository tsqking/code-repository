...
function returnDate(){
	...
	var thisYear=nowDate.getFullYear();    //获取完整的年份(4位)	
...
$(function() {
	...
	selectInitial("degree","../system/option!getOptionsByGPVal.do?value=DEGREE","3",false);
	selectInitial("degree_comment","../system/option!getOptionsByGPVal.do?value=DEGREE_COMT","1",false);
	selectInitial("engLevel","../system/option!getOptionsByGPVal.do?value=ENG_LEVEL","4",false);
...
$(document).on("click","#resetButton",
	function() {
		resetIndex=layer.open({
		    ...
		    content: confirm_reset_form,// var confirm_reset_form="确认要重置信息吗？";
		    btn: [confirm,cancel],
		    shadeClose: false,
		    yes: function(){
		    	resetInfo();
...
//重置信息
function resetInfo(){
	clearTable("addForm");//表单信息重置
	document.getElementById("preview").src = oldPhoto;//照片重置
	$("#resumeFile_name").text("");//简历附件重置
	if(document.all) {    // 此处判断是否是IE
	    $('#resume').replaceWith($('#resume').clone(true));
	} else {
	    $('#resume').val('');
	}
	$("#birthday").val("1995-01-01");//生日初始化
	$("#degree").val("3").trigger("change");
	$("#degree_comment").val("1").trigger("change");
	$("#engLevel").val("4").trigger("change");
	...
	if($("#resume").val()!=""){//不为空，需要上传
		...
		ajaxFileUpload(ids,'../campusRC/talent!uploadResume.do',2,"Resume",otherParam,
			function(data, status){
				...
				hideMask();
				layer.alert(addMessage,{"title":feedback});
				resetInfo();
			}
		);
	}else{
